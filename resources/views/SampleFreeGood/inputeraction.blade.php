...
<form action="{{url('SampleFreeGood/approve')}}" method="POST">
    ...
    <a href="{{url('SampleFreeGood/InputerView/'.$nextID)}}"><button type="button" class="btn btn-secondary">NEXT</button></a>
    @else
    <a href="{{url('SampleFreeGood/SampleFreeGoodList')}}"><button type="button" class="btn btn-secondary">NEXT</button></a>
    @endif
</form>
@else
<form action="{{url('SampleFreeGood/approve')}}" method="POST">
