...
<form action="{{url('SampleFreeGood/approve')}}" method="POST">
    @csrf
    <input type="hidden" name="approveID" value="{{isset($edit->id) && $edit->id!=''?$edit->id:''}}">
    <div class="tab-content" id="myTabContent">
        @if($edit->Approve_status!='APPROVE' && in_array(1, $STEP) || in_array(2, $STEP) || in_array(3, $STEP) || isset($EXT[10]['Forward']))
        <div class="button_div">
            <div class="selector">
                ...
                <div class="selecotr-item">
                    <input type="radio" id="radio10" name="pre_post_approval" class="selector-item_radio" value="QUERY">
                    <label for="radio10" class="selector-item_label">QUERY</label>
                </div>
            </div>
    ...
    <a href="{{url('SampleFreeGood/ApproverView/'.$nextID)}}"><button type="button" class="btn btn-secondary">NEXT</button></a>
    @else
    <a href="{{url('SampleFreeGood/SampleFreeGoodApproveList')}}"><button type="button" class="btn btn-secondary">NEXT</button></a>
    @endif
</form>
@endif
<script>
