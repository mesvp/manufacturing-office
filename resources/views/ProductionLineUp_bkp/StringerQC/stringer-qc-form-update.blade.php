    ...
    <div class="container-fluid flex-grow-1 container-p-y">
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center bg-label-primary py-1">
                <h5 class="mb-0">Stringer QC Update Request :</h5>
                <div class="text-end">
            ...
            <div class="card-body">
                <form action="{{ route('stringer-qc-update', $stringerQcDetails->id) }}" method="POST">
                    @csrf
                    <div class="col-lg-12 mx-auto">
                        <div class="row g-2">
                            <div class="col-md-2 col-sm-6 col-12">
                                <label for="batch_no" class="form-label">Batch No.</label>                            
                                <input class="form-control" type="text" id="batchNo" name="batchNo" value="{{ $stringerQcDetails->batchNo ?? '' }}" paceholder="" readonly>
                            </div>
                            <div class="col-md-2 col-sm-6 col-12">
                                <label for="date" class="form-label">Date</label>
                                <input type="text" id="date" name="date" placeholder="YYYY-MM-DD" class="form-control dob-picker"
                                    value="{{ $stringerQcDetails->date }}" required/>
                            </div>
                            <div class="col-md-2 col-sm-6 col-12">
                                ...
                                <div class="form-floating-outline">
                                    <select id="shift" name="shift" class="select2 form-select" required>
                                        @foreach ($ShiftMaster as $shift)
                                            <option value="{{ $shift->id }}" {{ $stringerQcDetails->shift == $shift->id ? 'selected' : '' }}>
                                                {{ $shift->shift }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-2 col-sm-6 col-12">
                                <label for="strNo" class="form-label">Select Stringer No</label>
                                <div class="form-floating-outline">
                                    <select id="strNo" name="strNo" class="select2 form-select" required>
                                        @foreach ($StringerNo as $stringer)
                                            <option value="{{ $stringer->mstr_type_name }}" {{ $stringerQcDetails->strNo == $stringer->mstr_type_name ? 'selected' : '' }}>
                                                {{ $stringer->mstr_type_name }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="form-floating-outline">
                                    <select id="operator" name="operator" class="select2 form-select" required>
                                        @foreach ($employees as $admin)
                                            <option value="{{ $admin->id }}" {{ $stringerQcDetails->operator == $admin->id ? 'selected' : '' }}>
                                                {{ $admin->fullname }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-2 col-sm-6 col-12">
                                ...
                                <div class="form-floating-outline">
                                    <select id="checker" name="checker" class="select2 form-select" required>
                                        @foreach ($employees as $admin)
                                            <option value="{{ $admin->id }}" {{ $stringerQcDetails->checker == $admin->id ? 'selected' : '' }}>
                                                {{ $admin->fullname }}
                                            </option>
                                        @endforeach
                                    </select>
...
@section('pageScript')
    <script>
        let selectedStringerNo = '{{ $stringerQcDetails->strNo }}';
        let materialIndex = 0;
        ...
        function addMaterialRow(material = {}) {
            ...
            const row = `
                <tr>
                    ...
                    <td>
                        <input type="text" name="materials[${materialIndex}][stringer_no]" class="form-control w-px-120" value="${material.stringer_no || ''}" placeholder="Stringer No" required readonly>
                    </td>
        ...
        $(document).ready(function() {
            ...
            @if (!empty($stringerQcMaterials) && count($stringerQcMaterials) > 0)
                @foreach ($stringerQcMaterials as $detail)
                    addMaterialRow({
                        id: '{{ $detail->id }}',
                        mat_id: '{{ $detail->mat_id }}',
                        stringer_no: '{{ $detail->stringerNo }}',
                        time: '{{ $detail->time }}',
