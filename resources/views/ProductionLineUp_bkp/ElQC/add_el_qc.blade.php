    ...
    <div class="container-fluid flex-grow-1 container-p-y">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center bg-label-primary py-2">
                <h5 class="mb-0">Add EL & Quality Control page : </h5>
                <div class="text-end">
            ...
            <div class="card-body">
                <form  id="elform" action="{{ url('production-lineup/el-qc-store') }}" method="POST">
                    @csrf
                                ...
                                <div class="form-floating form-floating-outline">
                                    @if (request()->get('page'))
                                        <select class="form-select select2" id="batch_No" name="batchNo"
                                            onchange="getBushId(this.value),getDefBatchId(this.value)" required>
                                ...
                                <tbody class="table-border-bottom-0">
                                    ...
                                    <tr>
                                        ...
                                        <td><input type="text" id="barcodeInput2" class="form-control" name="barCode"
                                                placeholder="Fetch No." autocomplete="off" required></td>
                                    </tr>
                                    <tr>
                                        <td>Wattage</td>
                                        <td class="py-1"> </td>
                    ...
                    <div class="col-12 text-end">
                        <button type="button" id="elsubmit" class="btn btn-primary ms-auto waves-effect waves-light mt-3">Submit</button>
                    </div>
...
@section('pageScript')
    ...
    @php
        ...
        $dmgReasonOptions = '<option selected>Select</option>';
        foreach ($DmgRsn ?? [] as $dmg) {
            $dmgReasonOptions .= "<option value=\"{$dmg->mstr_type_name}\">{$dmg->mstr_type_name}</option>";
        }

        $dmgCatOptions = '<option selected>Select</option>';
        foreach ($DmgCat ?? [] as $dmg) {
    ...
    <script>
        ...
        $(document).ready(function() {
            $(document).on("click", "#addProduct", function(e) {
                ...
                $row.find('select[name="dmgMat_reason[]"]').html(window.dmgReasonOptions || '<option>Select</option>');
                $row.find('select[name="dmgMat_cat[]"]').html(window.dmgCatOptions || '<option>Select</option>');
