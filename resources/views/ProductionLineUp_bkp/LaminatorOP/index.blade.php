...
@section('content')
    ...
    <div class="container-fluid flex-grow-1 container-p-y">
        <div class="card-header d-flex justify-content-between align-items-center py-2">
            <h5><span class="text-muted fw-light"> Production Set up /</span> Laminator OP Work</h5>
            ...
            <div class="col-md-12">
                <ul class="nav nav-pills flex-column flex-md-row mb-3 gap-2 gap-lg-0" role="tablist">
                    <li class="nav-item">
                        <a class="nav-link active" href="javascript:void(0);" role="tab" data-bs-toggle="tab"
                            ...
                            <i class="mdi mdi-account-outline mdi-20px me-1"></i>Pending For Lamination
                        </a>
                    </li>
                    <li class="nav-item">
                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center bg-label-primary py-2">
                        <h5 class="mb-0">Laminator OP List Details</h5>
                        <div class="text-end">
                            <a href="{{ route('laminator-op-add', ['page' => 'ALL']) }}" class="ms-2 btn  btn-primary btn-sm waves-effect waves-light"><span
                                    class="mdi mdi-playlist-plus me-1"></span> Add OP</a>
                        </div>
                    </div>
                    <div class="card-body">
                                        ...
                                        <tbody>
                                            @foreach ($AllLists as $item)
                                                <tr>
                                                    ...
                                                    <td>{{ $item->laminator_source ?? 'ELQC' }}</td>
                                                    <td>{{ $item->wattage ?? '-' }}</td>
                                                    ...
                                                    <td>
                                                        @if($item->rwrk_sts == '1' && $item->sts == '0')
                                                            <a href="{{ route('laminator-op-view', ['id' => $item->laminator_id]) }}?page=RWRK"
                                                               class="ms-2 btn btn-primary btn-sm waves-effect waves-light">
                                                                <span class="mdi mdi-playlist-plus me-1"></span> Add
                                                            </a>
                                                        @else
                                                            <a href="{{ route('laminator-op-add', ['id' => $item->elqc_batchNo, 'bid' => $item->elqc_id]) }}"
                                        ...
                                        <tbody>
                                            @foreach ($AllLaminatorLists as $item)
                                                @if ($item->status == '1' && $item->rwrk_status == '1')
                                                    <tr>
                                                        <td>{{ $loop->iteration }}</td>
                                                        <td>{{ \Carbon\Carbon::parse($item->laminator_date)->format('d/m/Y') }}
                                                        </td>
                                                        <td>{{ \Carbon\Carbon::parse($item->laminator_time)->format('h:i A') }}
                                                        </td>
                                                        <td>{{ $item->shiftdtl }}</td>
                                                        ...
                                                        <td>
                                                            <a class="btn btn-primary btn-xs text-capitalize waves-effect waves-light"
                                                                href="{{ route('laminator-op-view', ['id' => $item->laminator_id]) }}?page=VIEW" role="button"><i class="mdi mdi-eye"></i>
                                                                View</a>
                                                        </td>
                                                    </tr>
                                                @endif
                            ...
                            <div class="tab-pane fade" id="navs-reject" role="tabpanel">
                                <div class="">
                                    <table
                                        ...
                                        <tbody>
                                            @foreach ($AllLaminatorLists as $item)
                                                @if ($item->status == '2' || $item->rwrk_status == '2')
                                                    <tr>
                                                        <td>{{ $loop->iteration }}</td>
                                                        <td>{{ \Carbon\Carbon::parse($item->laminator_date)->format('d/m/Y') }}
                                                        </td>
