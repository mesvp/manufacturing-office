...
<form action="{{url('Production/approve')}}" method="POST">
    ...
    <a href="{{url('Production/InputerView/'.$nextID)}}"><button type="button" class="btn btn-secondary">NEXT</button></a>
    @else
    <a href="{{url('Production/ProductionList')}}"><button type="button" class="btn btn-secondary">NEXT</button></a>
    @endif
</form>
@else
<form action="{{url('Production/approve')}}" method="POST">
