...
@endphp
    @if ($type == 'in')
        @if($edit->Approve_status!='REJECT')
            <form action="{{url('GatePass/visitor_approve')}}" method="POST">
                @csrf
                ...
                <div class="tab-content" id="myTabContent">
                    @if($edit->Approve_status!='APPROVE' && (in_array(1, $CUSTSTEP) || in_array(2, $CUSTSTEP) || in_array(3, $CUSTSTEP) || isset($CUSTEXT[2]['Forward'])))
                        ...
                        <div id="Forwords" class="row" style="display: none;">
                            <div class="col-sm-4 form-group">
                                <label>Forward To</lable>
                                    <select class="form-select form-select-sm js-example-matcher-start requirrreddd" name="Forward_To">
                                        <option value="" selected disabled>Select</option>
                                        @foreach($visitorName as $val)
        ...
        @endif
    @else
        @if($edit->Out_Approve_status!='REJECT')
            <form action="{{url('GatePass/out_visitor_approve')}}" method="POST">
                @csrf
                <input type="hidden" name="type" value="{{isset($type) && $type!=''?$type:''}}">
                <input type="hidden" name="req_no" value="{{isset($req_no) && $req_no!=''?$req_no:''}}">
                <input type="hidden" name="approveID" value="{{isset($edit->id) && $edit->id!=''?$edit->id:''}}">
                <div class="tab-content" id="myTabContent">
                    @if($edit->Out_Approve_status!='APPROVE' && (in_array(1, $CUSTSTEP) || in_array(2, $CUSTSTEP) || in_array(3, $CUSTSTEP) || isset($CUSTEXT[2]['Forward'])))
                    <div class="button_div">
                        <div class="selector">
                            <div class="selecotr-item">
                                <input type="radio" id="radio1" name="during_approval" class="selector-item_radio" value="APPROVE" required>
