                ...
                <div class="row">
                    ...
                    <div class="row">
                       <div class="col-xl-6 col-lg-6 col-md-6 col-sm-12">
                            <div class="">
                                ...
                                <a href="{{url('Storeissue/StoreissueListClosed')}}" class="btn  btn-outline-default materialbtn btn-primary" id-data="MaterialIssueClosed">Material Issue Closed</a>
                                <a href="#" class="btn  btn-outline-default materialbtn" id-data="MaterialIssuedetail">Material Issue detail</a>
                            </div>
                        </div>
                        <div class="col-xl-6 col-lg-6 col-md-6 col-sm-12">
                            <div class="FilterButtonnn">
                                <div class="raone">
                                    <p class="MyToggle m-0 raho" id="MyToggle">
                                    ...
                                    <div class="ukom myFilter" id="myFilter">
                                        ...
                                        <div class="form-check">
                                            <input type="checkbox" class="form-check-input" id="Required_Quantity" value="Required Quantity" onclick="filterTable(this)">
                                            <label class="form-check-label" for="Required_Quantity">Required Quantity</label>
                                        </div>
                                        <div class="form-check">
                                            <input type="checkbox" class="form-check-input" id="Issue_Quantity" value="Issue Quantity" onclick="filterTable(this)">
                                            <label class="form-check-label" for="Issue_Quantity">Issue Quantity</label>
                                        </div>
                                        <div class="form-check">
                                            <input type="checkbox" class="form-check-input" id="Approver" value="Approver" onclick="filterTable(this)">
                     ...
                     <div class="row">
                        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 px-4">
                            <ul class="nav nav-tabs mt-2" id="ex1" role="tablist">
                                <li class="nav-item mr-1" role="presentation">
                                    <a class="nav-link count active" id="Alls" data-mdb-toggle="tab" href="#All" role="tab" aria-controls="All" aria-selected="false" tabindex="-1">All <span class="countss">{{$count['total']}}</span></a>
                                </li>
                                <li class="nav-item" role="presentation">
                                    <a class="nav-link count" id="Pendings" data-mdb-toggle="tab" href="#Pending" role="tab" aria-controls="Pending" aria-selected="false" tabindex="-1">My Issue Details
                                        <span class="countss">{{$count['mytotal']}}</span></a>
                                </li>
                            </ul>
                        </div>
                    </div>
                    <div class="tab-content" id="ex1-content">
                        <div class="tab-pane fade show active ml-0 mt-1" id="All" role="tabpanel" aria-labelledby="Alls">
                            <div class="table-responsive MaterialIssue" id="MaterialIssuedetail">
                                <table id="example2" class="table table-striped table-bordered" style="width:100%">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="th-sm">Issue Quantity</th> -->
                                    ...
                                    <tbody>
                                        ...
                                        @foreach($val->stroeissue as $issuedata)

                                        <tr>
                                            <td>{{$i++}}</td>
                                            <td>{{$val->Creater->fullname??'' }}</td>
                                            <td>{{$val->Request_No??'' }}</td>
                                            <td>{{$issuedata->material_data->issueNO??'' }}</td>
                                            {{-- <td>{{$val->Work_Order_No??'' }}</td> --}}
                                            <td>
                                                @if($issuedata->material_data->organisation)
                                                    {{$issuedata->material_data->organisation}}
                                                @else
                                            ...
                                            <!-- <td>{{$issuedata->Material_Name ?? '' }}</td>
                                            <td>{{$issuedata->QTY ?? '' }}</td>
                                            <td>{{$issuedata->material_data->issueQTY ?? '' }}</td> -->
                                            <td>{{$admindata[$issuedata->material_data->userID]??''}}</td>
                                            <td>{{$admindata[$issuedata->material_data->recived_by]??''}}</td>
                                            <td>{{$admindata[$issuedata->material_data->recived_ApproverID]??''}}</td>
                                            <td>

                                                {{status($issuedata->material_data->action)}}


                                            </td>
                                            <td>
                                                <a href="{{url('Storeissue/AddStoreissue/'.$val->id)}}"><button type="button" class="btn btn-primary">View</button></a>
                                                @if($issuedata->material_data->userID==auth()->user()->id && $issuedata->material_data->action=='RECHECK')
                                                <a href="{{url('Storeissue/ViewStoreissue/'.$val->id)}}/RECHECK/{{$issuedata->material_data->issueNO}}/{{$issuedata->material_data->orgid}}"><button type="button" class="btn btn-danger">Edit
                                                        </button></a>
                                                @elseif($issuedata->material_data->recived_ApproverID==auth()->user()->id && $issuedata->material_data->action=='HOLD')
                                                <a href="{{url('Storeissue/release/'.$val->id)}}/{{$issuedata->material_data->issueNO}}"><button type="button" class="btn btn-secondary">Release
                                                        </button></a>
                                                 @endif
                                            </td>
                                        </tr>
                        ...
                        <div class="tab-pane fade" id="Pending" role="tabpanel" aria-labelledby="Pendings">
                            <div class="table-responsive MaterialIssue" id="MaterialIssuePending">
                            <table id="" class="table table-striped table-bordered example" style="width:100%">
                                    ...
                                    <tbody>
                                        @php ($i=1)
                                        @foreach($myall as $key=>$val)
                                        @foreach($val->stroeissue as $issuedata)

                                        @if($issuedata->material_data->userID==auth()->user()->id)
                                        <tr>
                                            <td>{{$i++}}</td>
                                            <td>{{$val->Creater->fullname??'' }}</td>
                                            <td>{{$val->Request_No??'' }}</td>
                                            ...
                                            <td>
                                                <a href="{{url('Storeissue/AddStoreissue/'.$val->id)}}"><button type="button" class="btn btn-primary">View </button></a>
                                                @if($issuedata->material_data->userID==auth()->user()->id && $issuedata->material_data->action=='RECHECK')
                                                <a href="{{url('Storeissue/ViewStoreissue/'.$val->id)}}/RECHECK/{{$issuedata->material_data->issueNO}}/{{$issuedata->material_data->orgid}}"><button type="button" class="btn btn-secondary">Edit
                                                        </button></a>
                                                @endif
                                            </td>
                                        </tr>
