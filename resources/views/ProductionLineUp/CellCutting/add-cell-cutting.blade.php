    ...
    <div class="container-fluid flex-grow-1 container-p-y">
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center bg-label-primary py-1">
                <h5 class="mb-0">Cell Cutting Add Request :</h5>
                <div class="text-end">
            ...
            <div class="card-body">
                <form action="{{ route('cell-cutting-store') }}" method="POST">
                    @csrf
                    <div class="col-lg-12 mx-auto">
                        <div class="row g-2">
                            <div class="col-md-2 col-sm-6 col-12">
                                <label for="defaultInput" class="form-label">Batch No.</label>
                                <div class="form-floating-outline">
                                    <select id="" name="batch_no" class="select2 form-select"
                            ...
                            <div class="col-md-2 col-sm-6 col-12">
                                <label for="defaultInput" class="form-label">Select Shift</label>
                                <div class="form-floating-outline">
                                    <select id="" name="shift" class="select2 form-select" required>
                            ...
                            <div class="col-md-2 col-sm-6 col-12">
                                <label for="defaultInput" class="form-label">Plant No.</label>
                                <input class="form-control" type="text" name="plant_no" id="plant_no"
                                    placeholder="Plant No." disabled>
                            </div>
                            <div class="col-md-2 col-sm-6 col-12">
                                <label for="defaultInput" class="form-label">Wattage</label>
                                <input class="form-control" type="text" id="wattage" name="wattage" placeholder="Wattage"
                                    disabled>
                            </div>
                            <!--<div class="col-md-2 col-sm-6 col-12">-->
                            <!--    <label for="defaultInput" class="form-label">Brand </label>-->
                            <!--    <input class="form-control" type="text" id="brand" name="brand" placeholder="Brand"-->
                            <!--        disabled>-->
                            <!--</div>-->

                            <div class="col-md-2 col-sm-6 col-12">
                                <label for="defaultInput" class="form-label">Efficiency</label>
                            ...
                            <div class="col-md-4 col-sm-6 col-12">
                                <label for="defaultInput" class="form-label">Select Operator</label>
                                <div class="form-floating-outline">
                                    <select id="" name="operator" class="select2 form-select" required>
                            ...
                            <div class="col-md-4 col-sm-6 col-12">
                                <label for="defaultInput" class="form-label">Select Checker</label>
                                <div class="form-floating-outline">
                                    <select id="" name="checker" class="select2 form-select" required>
    ...
    <script>
        ...
        function addMaterialRow(material = {}) {
            ...
            <tr>
                ...
                <td><select class="form-select select2-dynamic w-px-25" name="materials[${materialIndex}][name]" required>${materialOptions(material.id || '')}</select></td>
                ...
                <td><input type="number" min="0" step="0.01" inputmode="decimal" name="materials[${materialIndex}][production_qty]" class="form-control invoice-item-price w-px-12" placeholder="Production Qty" required></td>
                <td><input type="number" min="0" step="0.01" inputmode="decimal" name="materials[${materialIndex}][rejection_qty]" class="form-control invoice-item-price w-px-12" placeholder="Rejection Qty" required></td>
                <td><select class="form-select select2-dynamic w-px-200" name="materials[${materialIndex}][stage]" required>
                    <option value="" selected>Select Stage</option>
                    @foreach ($DmgRsn as $dmg)
                        <option value="{{ $dmg->mstr_type_name }}">{{ $dmg->mstr_type_name }}</option>
                    @endforeach
                </select></td>
                <td><select class="form-select select2-dynamic w-px-200" name="materials[${materialIndex}][defect_category]" required>
        ...
        function showHint(batchNo) {
            if (!batchNo) {
                $('#productionTable').html(
                    `<tr><td colspan="8" class="text-center text-danger">-- No Batch No. is Selected --</td></tr>`);
                return;
            }

            $.ajax({
                url: "{{ url('production-lineup/cell-cutting/getCellCuttingMaterial') }}",
                type: "GET",
                data: {
                    q: batchNo
                },
                success: function(response) {
                    ...
                    $('#brand').val(response.cell_company);
                ...
                error: function(xhr) {
                    ...
                    $('#productionTable').html(
                        `<tr><td colspan="8" class="text-center text-danger">No Data Found. Please try again.</td></tr>`
                    );
                }
            });
        }
        ...
        $(document).ready(function() {
            $('#productionTable').html(`
				<tr>
					<td colspan="8" class="text-center text-danger"> -- No Batch No. is Selected --</td>
				</tr>
			`);
        });
    </script>
