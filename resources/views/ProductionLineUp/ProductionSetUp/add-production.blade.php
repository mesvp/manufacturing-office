        ...
        <div class="col-lg-12 mx-auto">
          <div class="row g-2">
            ...
            <div class="col-md-3 col-sm-6 col-12">
              ...
              <select class="select2 form-select" name="finished_good" onchange="showBomMat(this.value)" required>
            ...
            <tbody id="productionTable">
              @foreach ($MaterialList as $key=>$material)
                <tr>
                    ...
                    <td>
                        <input type="text" class="form-control w-px-120 size-field" name="size[]"  required > <!--inputmode="decimal" pattern="[0-9.]+"
                    ...
                    <td>
                      <select name="bom_material[]" class="form-select w-px-200 bom-select" onchange="showBomMatQty(this)" required>
                        <option value="">Select BOM Material</option>
                      </select>
                    </td>
                    <td>
                        <input type="text" class="form-control brand-field bom-qty" name="bom_qty[]" readonly required >
                    </td>
                    <td>
                        <select name="useStage[]" class="form-select w-px-200 " required>
                        <option value="">Select Stage</option>
                        @foreach($ProcessStage as $stageData)
                        <option value="{{ $stageData->stage_name }}">{{ $stageData->stage_name }}</option>
                        @endforeach
                      </select>
                    </td>
                </tr>
...
<script>
    $(document).ready(function() {
        ...
        $('#viewMatrixBtn').on('click', function() {
            ...
            //  Rows with LETTER headers
            for (let i = 0; i < rows; i++) {
                const rowLetter = getColumnLetter(i); // A, B, C...
                matrixHtml += `<tr>
            <td class="p-1 header-cell text-light bg-facebook">${rowLetter}</td>`;

                for (let j = 0; j < cols; j++) {
                    matrixHtml += `
                <td class="p-1 matrix-cell"
                    data-toggle="tooltip"
                    title="Cell: ${rowLetter}${j + 1}">
                </td>`;
                }
                matrixHtml += '</tr>';
            }
...
<script>
function showBomMat(str) {
  if (!str) {
    $(".bom-select").html('<option value="">Select BOM Material</option>');
    return;
  }

  $.ajax({
    url: "{{ url('production-lineup/production-setup/ajaxBomMat') }}",
    type: "GET",
    data: { q: str },
    success: function(response) {
      // This updates EVERY row's BOM dropdown at once
      $(".bom-select").html(response);
    },
    error: function(xhr) {
      console.error("Error fetching BOM data:", xhr.responseText);
    }
  });
}


function showBomMatQty(element) {
    var materialId = $(element).val();
    // Find the 'bom-qty' input that exists in the same row as the select box
    var $rowInput = $(element).closest('tr').find(".bom-qty");
    ...
    $.ajax({
        url: "{{ url('production-lineup/production-setup/ajaxBomMatQty') }}",
        type: "GET",
        data: { q: materialId },
        success: function(response) {
            // Update ONLY the input in this specific row
            // If response is an HTML option string, use .html(), if it's a value, use .val()
            $rowInput.val(response); 
        },
        error: function(xhr) {
            console.error("Error fetching BOM data:", xhr.responseText);
        }
