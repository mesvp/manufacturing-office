...
@section('pageHeading')
    Add Request page (Stringer Re-Work) :
@endsection
    ...
    <div class="container-fluid flex-grow-1 container-p-y">
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center bg-label-primary py-1">
                <h5 class="mb-0">Add Request page (Stringer Re-Work) :</h5>
                <div class="text-end">
            ...
            <div class="card-body">
                <form action="{{ route('stringer-rework-store') }}" method="POST">
                    @csrf
                        ...
                        <div class="row g-2">
                            <div class="col-md-2 col-sm-6 col-12">
                                ...
                                <div class="form-floating-outline">
                                    <select id="batch_no" name="batch_no" class="select2 form-select"
                            ...
                            <div class="col-md-2 col-sm-6 col-12">
                                ...
                                <div class="form-floating-outline">
                                    <select id="plant_no" name="plant_no" class="select2 form-select" required>
                                        <option value="">Select Plant</option>
                                        @foreach ($PlantMaster as $plant)
                                            <option value="{{ $plant->mstr_type_name }}">{{ $plant->mstr_type_name }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
...
@section('pageScript')
    <script>
        ...
        function addMaterialRow(material = {}) {
            // First remove any "no data" message if it exists
        ...
        <tr>
            ...
            <td><input type="time" name="materials[${materialIndex}][time]" class="form-control invoice-item-price w-px-12" placeholder="Time" value="${material.time || ''}" required></td>
            <td><input type="text" name="materials[${materialIndex}][cell_position]" class="form-control invoice-item-price w-px-120" placeholder="Cell Position" pattern="[A-Za-z0-9]+" oninput="this.value = this.value.replace(/[^A-Za-z0-9]/g, '')" required></td>
            <td><input type="number" min="0" name="materials[${materialIndex}][production_qty]" class="form-control invoice-item-price w-px-150" placeholder="Production Qty" value="${material.production_qty || ''}" required></td>
            <td><input type="number" min="0" name="materials[${materialIndex}][rejection_qty]" class="form-control invoice-item-price w-px-150" placeholder="Rejection Qty" value="${material.rejection_qty || ''}" required></td>
            <td><select class="form-select w-px-200" name="materials[${materialIndex}][stage]" required>
                <option value="">Select Stage</option>
                @foreach ($DmgRsn as $dmg)
                    <option value="{{ $dmg->mstr_type_name }}" ${material.stage === '{{ $dmg->mstr_type_name }}' ? 'selected' : ''}>{{ $dmg->mstr_type_name }}</option>
                @endforeach
            </select></td>
            <td><select class="form-select w-px-200" name="materials[${materialIndex}][defect_category]" required>
                ...
                @foreach ($DefectCat as $cat)
                    <option value="{{ $cat->mstr_type_name }}" ${material.defect_category === '{{ $cat->mstr_type_name }}' ? 'selected' : ''}>{{ $cat->mstr_type_name }}</option>
                @endforeach
            </select></td>
        ...
        function removeRow(button) {
            ...
            if ($('#productionTable tr').length === 0) {
                ...
                // Ensure the first row has Add button
        ...
        function showHint(batchNo) {
            ...
            $.ajax({
                url: "{{ url('production-lineup/stringer-rework/getstringerReWorkMaterial') }}",
                type: "GET",
                data: {
                    q: batchNo
                },
