...
<form action="{{url('QCSampleTesting/approve')}}" method="POST">
    ...
    <a href="{{url('QCSampleTesting/InputerView/'.$nextID)}}"><button type="button" class="btn btn-secondary">NEXT</button></a>
    @else
    <a href="{{url('QCSampleTesting/STDFinishedGoodsList')}}"><button type="button" class="btn btn-secondary">NEXT</button></a>
    @endif
</form>
@else
<form action="{{url('QCSampleTesting/approve')}}" method="POST">
