...
<div class="card">
    <div class="app-content">
        ...
        <section class="section">
            <ol class="breadcrumb">
                <li class="breadcrumb-item">Plant Stock Report Details</li>
            </ol>
            ...
            <div class="row">
                <div class="container">
                        ...
                        <div class="row">
                            ...
                            <div class="col-xl-3 col-lg-6 col-md-6 col-sm-12">
                                <div class="addbtn extra p-0">
                                    <a href="{{url('Report/storestockreport')}}" class="btn btn-info mr-1 btn-sm"> <i class="fa fa-arrow-left"></i></a>
                                    <a href="{{url('Report/storestockreport')}}" class="btn btn-info btn-sm"> <i class="fa fa-home"></i></a>
                                </div>
                            </div>
                        </div>
                    </div>
                    <form action="{{url('Report/filtered-details-plant/'. $matid)}}" method="POST">
                        @csrf
                        <div class="row filter">
                            ...
                            <div class="col-2 mb-3">
                                ...
                                <select name="Material_Name" class="form-select form-select-sm js-example-matcher-start">
                                    ...
                                    <option value="{{ isset($val->id) && $val->id!=''?$val->id:'' }}" {{isset($matid) && $matid==$val->id?'selected':''}}>{{ isset($val->material_name) && $val->material_name!=''?$val->material_name:''}}</option>
                                    @endforeach
                                    @endif
                                    @if(isset($Materialss))
                                    @foreach($Material_Name as $val)
                            ...
                            <div class="col-2 mt-4">
                                ...
                                <a href="{{ url('Report/plantstockreportdetails/' . $matid) }}"><button type="button" class="btn btn-secondary"><i class="fa fa-refresh"></i></button></a>
                            </div>
                    ...
                    <div class="col-xl-12 col-lg-12 col-md-12 text-center">
                        <h6 class="text-center p-1" style="background-color: #e5e5e5;display: inline;">Plant Stock Report Details</h6>
                    </div>
                    <br>
                                ...
                                <table id="example2" class="table table-sm table-striped table-bordered">
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($allMaterials as $key => $val)
                                            ...
                                            @if ($isReceived)
                                                @php
                                                    // For received materials (Store Issues to Plant)
                                                    if (isset($val->Store_Requistion_id) && $val->Store_Requistion_id != null) {
                                                        // Store Issue data - show from store data, to plant data
                                                        $fromProject = isset($val->pname) && $val->pname != '' ? $val->pname : '0';
                                                        $fromSubProject = isset($val->spname) && $val->spname != '' ? $val->spname : '0';
                                                        $orgFrom = isset($val->organisation) && $val->organisation != '' ? $val->organisation : '0';
                                                        $godownFrom = isset($val->inventory_name) && $val->inventory_name != '' ? $val->inventory_name : '0';
                                                        
                                                        $receivedQty = isset($val->issueQTY) && $val->issueQTY != '' ? $val->issueQTY : 0;
                                                        
                                                        $lastPurchase = App\Models\Master\RawMaterial\Master_Raw_Material::select('Rate','GST')
                                                                        ->where('Material', $val->Material_id)
                                                                        ->orderBy('id', 'DESC')
                                                                        ->first();
                                                        $last_purchase_price = $lastPurchase ? $lastPurchase->Rate : null;
                                                        $last_purchase_gst = $lastPurchase ? $lastPurchase->GST : null;
                                                        $receiveAmt = $val->issueQTY * ($last_purchase_price + $last_purchase_gst / 100);
                                                        
                                                        $transactionDate = isset($val->created_at) && $val->created_at != '' ? $val->created_at : '';
                                                        $narration = "Received From Store Issue";
                                                        
                                                        $totalIssueQty += $receivedQty;
                                                        $totalIssuePrice += $receiveAmt;
                                                    } else {
                                                        // Production data - this is material received from production - show in FROM columns
                                                        $fromProject = isset($val->pname) && $val->pname != '' ? $val->pname : 'Production';
                                                        $fromSubProject = isset($val->spname) && $val->spname != '' ? $val->spname : 'Plant';
                                                        $orgFrom = isset($val->organisation) && $val->organisation != '' ? $val->organisation : '0';
                                                        $godownFrom = '0'; // Set godown as Production for clarity

                                                        $receivedQty = isset($val->issueQTY) && $val->issueQTY != '' ? $val->issueQTY : 0;

                                                        $lastPurchase = App\Models\Master\RawMaterial\Master_Raw_Material::select('Rate','GST')
                                                                        ->where('Material', $val->Raw_Material)
                                                                        ->orderBy('id', 'DESC')
                                                                        ->first();
                                                        $last_purchase_price = $lastPurchase ? $lastPurchase->Rate : null;
                                                        $last_purchase_gst = $lastPurchase ? $lastPurchase->GST : null;
                                                        ...
                                                        $transactionDate = isset($val->Production_Date) && $val->Production_Date != '' ? $val->Production_Date : '';
                                                        $narration = "Received From Production";
                                            ...
                                            @if ($isIssued)
                                                @php
                                                    // For issued materials (Production Data - materials consumed in production)
                                                    $toProject = isset($val->pname) && $val->pname != '' ? $val->pname : '0';
                                                    $toSubProject = isset($val->spname) && $val->spname != '' ? $val->spname : '0';
                                                    $orgTo = isset($val->organisation) && $val->organisation != '' ? $val->organisation : '0';
                                                    
                                                    $issueQty = isset($val->TotalQty) && $val->TotalQty != '' ? $val->TotalQty : 0;
                                                    
                                                    $lastPurchase = App\Models\Master\RawMaterial\Master_Raw_Material::select('Rate','GST')
                                                                    ->where('Material', $val->RawMaterial_id)
                                                                    ->orderBy('id', 'DESC')
                                                                    ->first();
                                                    $last_purchase_price = $lastPurchase ? $lastPurchase->Rate : null;
                                                    $last_purchase_gst = $lastPurchase ? $lastPurchase->GST : null;
                                                    $issueAmt = $val->TotalQty * ($last_purchase_price + $last_purchase_gst / 100);
                                                    
                                                    $transactionDate = isset($val->created_at) && $val->created_at != '' ? $val->created_at : '';
                                                    $narration = "Consumed in Production";
                                                    
                                                    $totalQuantity += $issueQty;
                                                    $totalReceivedPrice += $issueAmt;
                                                @endphp
                                            @endif
                                            ...
                                            <tr>
                                                ...
                                                <td>{{ !empty($receiveAmt) ? number_format($receiveAmt, 2) : 0 }}</td>
                                                <td>{{ !empty($issueQty) ? $issueQty : 0 }}</td>
                                                <td>{{ !empty($issueAmt) ? number_format($issueAmt, 2) : 0 }}</td>
                                    ...
                                    <!-- Move totals row to tfoot for better table structure -->
                                    <tfoot>
                                        <tr class="table-info">
                                            ...
                                            <td><strong>{{ number_format($totalReceivedPrice, 2) }}</strong></td>
                                            <td></td>
                                            <td></td>
                                        </tr>
                                    </tfoot>
