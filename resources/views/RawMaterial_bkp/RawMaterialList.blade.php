...
<div class="card">
    <div class="app-content">
        ...
        <section class="section">
            <ol class="breadcrumb">
                <li class="breadcrumb-item">Raw Material View Page</li>
            </ol>
            <div class="addbtn">
                <a href="{{url('RawMaterial/ExportFilteredData')}}"><i class='fas fa-file-excel'></i></a>
                @if(isset($EXT[6]['inputer']))
                <a href="{{url('RawMaterial/RawMaterial')}}"><button class="btn btn-info">Add Raw Material</button></a>
                @endif
            </div>
            <div class="row">
                <div class="container">
                    <form action="{{url('RawMaterial/filtered')}}" method="POST">
                        @csrf
                        <div class="row filter">
                            ...
                            <div class="col-2 mb-3">
                                ...
                                <select name="Organization" class="form-select form-select-sm js-example-matcher-start">
                                    ...
                                    <option value="all" {{isset($Organizationss) && $Organizationss === 'all' ? 'selected' : '' }}>All</option>
                                    <?php $RepeatData = []; ?>
                                    @foreach($Organization as $val)
                                    <option value="{{ isset($val->id) && $val->id!=''?$val->id:'' }}" {{isset($Organizationss) && $Organizationss==$val->id?'selected':''}}>{{ isset($val->organization) && $val->organization!=''?$val->organization:''}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-2 mb-3">
                                ...
                                <select name="Manufacturing_Unit" class="form-select form-select-sm js-example-matcher-start">
                                    ...
                                    <?php $RepeatData = []; ?>
                                    @foreach($ManufacturingUnit as $val)
                            ...
                            <div class="col-2 mb-3">
                                ...
                                <select name="Godown_Name" class="form-select form-select-sm js-example-matcher-start">
                                    <option value="" disabled selected>Select</option>
                                    <option value="all" {{isset($GodownNames) && $GodownNames === 'all' ? 'selected' : '' }}>All</option>
                                    <?php $RepeatData = []; ?>
                                    @foreach($GodownName as $val)
                                    <option value="{{ isset($val->id) && $val->id!=''?$val->id:'' }}" {{isset($GodownNames) && $GodownNames==$val->id?'selected':''}}>{{ isset($val->Godown_Name) && $val->Godown_Name!=''?$val->Godown_Name:''}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-2 mb-3">
                                ...
                                <select name="Raw_Material" class="form-select form-select-sm js-example-matcher-start">
                                    ...
                                    <option value="{{ isset($val->id) && $val->id!=''?$val->id:'' }}" {{isset($RawMaterialss) && $RawMaterialss==$val->id?'selected':''}}>{{ isset($val->Material_Name) && $val->Material_Name!=''?$val->Material_Name:''}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-2 mb-3">
                                ...
                                <select name="HSN_Code" class="form-select form-select-sm js-example-matcher-start">
                                    ...
                                    <?php
                                    $HSNCode = isset($val->rawss->HSN_Code) && $val->rawss->HSN_Code != '' ? $val->rawss->HSN_Code : '';
                            ...
                            <div class="col-2 mb-3">
                                <label for="" class="form-label">OB</label>
                                <select name="OB" class="form-select form-select-sm js-example-matcher-start">
                                    <option value="" disabled selected>Select</option>
                                    <option value="all" {{isset($OBss) && $OBss === 'all' ? 'selected' : '' }}>All</option>
                                    <?php $RepeatData = []; ?>
                                    @foreach($OB as $val)
                                    <option value="{{ isset($val->id) && $val->id!=''?$val->id:'' }}" {{isset($OBss) && $OBss==$val->id?'selected':''}}>{{ isset($val->OB) && $val->OB!=''?$val->OB:''}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-2 mb-3">
                                <label for="" class="form-label">Received QTY</label>
                                <select name="Received_QTY" class="form-select form-select-sm js-example-matcher-start">
                                    <option value="" disabled selected>Select</option>
                                    <option value="all" {{isset($ReceivedQTYs) && $ReceivedQTYs === 'all' ? 'selected' : '' }}>All</option>
                                    <?php $RepeatData = []; ?>
                                    @foreach($DropdownData as $val)
                                    <?php
                                    $ReceivedQTY = isset($val->rawss->Received_QTY) && $val->rawss->Received_QTY != '' ? $val->rawss->Received_QTY : '';
                                    if (!empty($ReceivedQTY) && !in_array($ReceivedQTY, $RepeatData)) {
                                        $RepeatData[] = $ReceivedQTY;
                                    ?>
                                        <option value="{{ $ReceivedQTY }}" {{isset($ReceivedQTYs) && $ReceivedQTYs==$ReceivedQTY?'selected':''}}>{{ $ReceivedQTY }}</option>
                                    <?php } ?>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-2 mb-3">
                                <label for="" class="form-label">Balance Stock</label>
                                <select name="Balance_Stock" class="form-select form-select-sm js-example-matcher-start">
                                    <option value="" disabled selected>Select</option>
                                    <option value="all" {{isset($BalanceStocks) && $BalanceStocks === 'all' ? 'selected' : '' }}>All</option>
                                    <?php $RepeatData = []; ?>
                                    @foreach($DropdownData as $val)
                                    <?php
                                    $BalanceStock = isset($val->rawss->Balance_Stock) && $val->rawss->Balance_Stock != '' ? $val->rawss->Balance_Stock : '';
                                    if (!empty($BalanceStock) && !in_array($BalanceStock, $RepeatData)) {
                                        $RepeatData[] = $BalanceStock;
                                    ?>
                                        <option value="{{ $BalanceStock }}" {{isset($BalanceStocks) && $BalanceStocks==$BalanceStock?'selected':''}}>{{ $BalanceStock }}</option>
                                    <?php } ?>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-2 mt-4">
                                ...
                                <a href="{{url('RawMaterial/RawMaterialList')}}"><button type="button" class="btn btn-secondary"><i class="fa fa-refresh"></i></button></a>
                            </div>
                            <div class="FilterButtonnn">
                                <div class="raone">
                                    ...
                                    <div class="ukom" id="myFilter">
                                        ...
                                        <div class="form-check">
                                            <input type="checkbox" class="form-check-input" id="Raw_Material" value="Raw Material" onclick="filterTable(this)">
                                        ...
                                        <div class="form-check">
                                            <input type="checkbox" class="form-check-input" id="OB" value="OB" onclick="filterTable(this)">
                                            <label class="form-check-label" for="OB">OB</label>
                                        </div>
                                        <div class="form-check">
                                            <input type="checkbox" class="form-check-input" id="Received_QTY" value="Received QTY" onclick="filterTable(this)">
                                            <label class="form-check-label" for="Received_QTY">Received QTY</label>
                                        </div>
                                        <div class="form-check">
                                            <input type="checkbox" class="form-check-input" id="Balance_Stock" value="Balance Stock" onclick="filterTable(this)">
                                            <label class="form-check-label" for="Balance_Stock">Balance Stock</label>
                                        </div>
                                        <div class="form-check">
                    ...
                    <ul class="nav nav-tabs mb-3" id="ex1" role="tablist">
                        <li class="nav-item" role="presentation">
                            <a class="nav-link count active" id="Alls" data-mdb-toggle="tab" href="#All" role="tab" aria-controls="All" aria-selected="true">All <span class="countss">{{count($rowStock)}}</span></a>
                        </li>
                        <li class="nav-item" role="presentation">
                                ...
                                <table class="table table-striped table-bordered example" style="width:100%">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="th-sm">Raw Material</th>
                                            <th class="th-sm">HSN Code</th>
                                            <th class="th-sm">UOM</th>
                                            <th class="th-sm">OB</th>
                                            <th class="th-sm">Received QTY</th>
                                            <th class="th-sm">Balance Stock</th>
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>{{isset($val->raw->date) && $val->raw->date!=''?$val->raw->date:''}}</td>
                                            <td>{{isset($val->Raw_Material->Material_Name) && $val->Raw_Material->Material_Name!=''?$val->Raw_Material->Material_Name:''}}</td>
                                            <td>{{isset($val->rawss->HSN_Code) && $val->rawss->HSN_Code!=''?$val->rawss->HSN_Code:''}}</td>
                                            <td>{{isset($val->UOM->UOMs) && $val->UOM->UOMs!=''?$val->UOM->UOMs:''}}</td>
                                            <td>{{isset($val->OB->OB) && $val->OB->OB!=''?$val->OB->OB:''}}</td>
                                            <td>{{isset($val->rawss->Received_QTY) && $val->rawss->Received_QTY!=''?$val->rawss->Received_QTY:''}}</td>
                                            <td>{{isset($val->rawss->Balance_Stock) && $val->rawss->Balance_Stock!=''?$val->rawss->Balance_Stock:''}}</td>
                                            ...
                                            <td class="maindffd">
                                                @if(isset($val->status) && $val->status!=1)
                                                <a href="{{url('RawMaterial/RawMaterial_View/'.$val->id.'/ALL')}}" class="btn btn-primary">View</a>
                                                @if($val->Approve_status == 'RECHECK' && isset($EXT[6]['inputer']))
                                                <a href="{{url('RawMaterial/RawMaterial/'.$val->id)}}" class="btn btn-secondary">Edit</a>
                                                @elseif($val->HoldStatus > 0)
                                                <a href="{{url('RawMaterial/Release_Hold/'.$val->id)}}" class="btn btn-secondary">Release</a>
                                                @endif
                                                @else
                                                <a href="{{url('RawMaterial/RawMaterial/'.$val->id)}}" class="btn btn-warning">Draft</a>
                                                @endif
                                            </td>
                                        </tr>
                                        @endforeach
                                ...
                                <table class="table table-striped table-bordered example" style="width:100%">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td class="maindffd">
                                                @if(isset($val->status) && $val->status!=1)
                                                <a href="{{url('RawMaterial/RawMaterial_View/'.$val->id.'/APPROVE')}}" class="btn btn-primary">View</a>
                                                @if($val->Approve_status == 'RECHECK' && isset($EXT[6]['inputer']))
                                                <a href="{{url('RawMaterial/RawMaterial/'.$val->id)}}" class="btn btn-secondary">Edit</a>
                                                @endif
                                                @else
                                ...
                                <table class="table table-striped table-bordered example" style="width:100%">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td class="maindffd">
                                                @if(isset($val->status) && $val->status!=1)
                                                <a href="{{url('RawMaterial/RawMaterial_View/'.$val->id.'/PENDING')}}" class="btn btn-primary">View</a>
                                                @if($val->Approve_status == 'RECHECK' && isset($EXT[6]['inputer']))
                                                <a href="{{url('RawMaterial/RawMaterial/'.$val->id)}}" class="btn btn-secondary">Edit</a>
                                                @endif
                                                @else
                                ...
                                <table class="table table-striped table-bordered example" style="width:100%">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td class="maindffd">
                                                @if(isset($val->status) && $val->status!=1)
                                                <a href="{{url('RawMaterial/RawMaterial_View/'.$val->id.'/HOLD')}}" class="btn btn-primary">View</a>
                                                @if($val->Approve_status == 'RECHECK' && isset($EXT[6]['inputer']))
                                                <a href="{{url('RawMaterial/RawMaterial/'.$val->id)}}" class="btn btn-secondary">Edit</a>
                                                @elseif($val->HoldStatus > 0)
                                                <a href="{{url('RawMaterial/Release_Hold/'.$val->id)}}" class="btn btn-secondary">Release</a>
                                ...
                                <table class="table table-striped table-bordered example" style="width:100%">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td class="maindffd">
                                                @if(isset($val->status) && $val->status!=1)
                                                <a href="{{url('RawMaterial/RawMaterial_View/'.$val->id.'/RECHECK')}}" class="btn btn-primary">View</a>
                                                @if($val->Approve_status == 'RECHECK' && isset($EXT[6]['inputer']))
                                                <a href="{{url('RawMaterial/RawMaterial/'.$val->id)}}" class="btn btn-secondary">Edit</a>
                                                @endif
                                                @else
                                ...
                                <table class="table table-striped table-bordered example" style="width:100%">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td class="maindffd">
                                                @if(isset($val->status) && $val->status!=1)
                                                <a href="{{url('RawMaterial/RawMaterial_View/'.$val->id.'/OBJECT')}}" class="btn btn-primary">View</a>
                                                @if($val->Approve_status == 'RECHECK' && isset($EXT[6]['inputer']))
                                                <a href="{{url('RawMaterial/RawMaterial/'.$val->id)}}" class="btn btn-secondary">Edit</a>
                                                @endif
                                                @else
                                ...
                                <table class="table table-striped table-bordered example" style="width:100%">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td class="maindffd">
                                                @if(isset($val->status) && $val->status!=1)
                                                <a href="{{url('RawMaterial/RawMaterial_View/'.$val->id.'/REJECT')}}" class="btn btn-primary">View</a>
                                                @if($val->Approve_status == 'RECHECK' && isset($EXT[6]['inputer']))
                                                <a href="{{url('RawMaterial/RawMaterial/'.$val->id)}}" class="btn btn-secondary">Edit</a>
                                                @endif
                                                @else
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        fetch("{{ url('RawMaterial/getCheckBoxData') }}?ID=" + tableID, {
                method: 'GET',
            })
            .then(response => {
                if (!response.ok) {
...
<script>
    $(document).ready(function() {
        $.ajax({
            url: "{{ url('RawMaterial/CheckHoldExpiry') }}",
            method: 'GET',
                ...
                response.forEach(function(lead) {
                    if (lead.action === 'HOLD' && lead.status === 1) {
                        ...
                        if (holdDate < currentDate) {
                            UpdateStatus(lead.RawMaterial_stock__id, lead.userID);
                        }
                    }
                });
            },
    ...
    function UpdateStatus(RawMaterial_stock__id, userID) {
        $.ajax({
            url: "{{ url('RawMaterial/UpdateStatus') }}",
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
            ...
            data: JSON.stringify({
                RawMaterial_stock__id: RawMaterial_stock__id,
                userID: userID
            }),
            success: function(response) {
                $('#statuss' + RawMaterial_stock__id).html('<span style="color: #FF9000;">Pending</span>');
            }
        });
    }
</script>
