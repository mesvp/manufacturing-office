...
namespace App\Http\Controllers\ProductionLineUp\StringerReWork;

use App\Http\Controllers\Controller;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;
use App\Models\Admin;
use App\Models\ProductionLineUp\Factory_Stringer_ReWork;
use App\Models\ProductionLineUp\Factory_Stringer_ReWork_Material;
use App\Models\ProductionLineUp\Factory_Stringer_ReWork_History;
use App\Models\ApprovalMatrix\ApprovalStage_Model;

class StringerReWork_Controller extends Controller
{
  ...
  public  static function PermittedMenuList($sessionId){
	...
	public function getstringerReWorkMaterial(Request $request)
	{
	...
	{
		$data['menu'] = 'stringer-rework';
		$data['empId'] = request()->session()->get('empId');

		$Cond = [];
		$Condition = 'prdm.material = 1 AND prdm2.material = 2';
		...
		$sql = "SELECT 
            ...
            FROM tbl_factory_stringer_rw_laravel AS psl
            INNER JOIN tbl_factory_stringer_rw_material_laravel AS psml 
                ON psml.StringerRwId = psl.id
            ...
            LEFT JOIN (
                SELECT phl1.*
                FROM tbl_factory_stringer_rw_history_laravel AS phl1
                WHERE phl1.id = (
                    SELECT MAX(phl2.id)
                    FROM tbl_factory_stringer_rw_history_laravel AS phl2
                    WHERE phl2.strReworkId = phl1.strReworkId
                )
            ) AS phl 
                ON phl.strReworkId = psl.id
		...
		/*
		$data['AllLists'] = DB::table('tbl_factory_stringer_rw_laravel as psl')
			->select(
				'psl.*',
				'prd.wattage',
				'asl.stage_title',
			...
			->join('tbl_factory_stringer_rw_material_laravel as psml', 'psml.StringerRwId', '=', 'psl.id')
			->join('tbl_factory_appr_stage_laravel as asl', 'psl.stage', '=', 'asl.id')
			...
			->leftjoin('tbl_factory_production_setup_material_laravel as prdm2', 'prdm2.batchNo', '=', 'psl.batchNo')
			->leftJoinSub(
				DB::table('tbl_factory_stringer_rw_history_laravel as phl1')
					->select('phl1.*')
					->whereRaw('phl1.id = (
            SELECT MAX(phl2.id)
            FROM tbl_factory_stringer_rw_history_laravel as phl2
			...
			->where('prdm.material', 1)
			->where('prdm2.material', 2)
			->orderBy('psl.created_at', 'DESC')
			->groupBy('psml.StringerRwId')
			->get();
		*/
		...
		$data['approverDetails'] = DB::table('tbl_factory_appr_laravel AS al')
			...
			->where('asl.stage_module', '1753935876')
		...
		return view('ProductionLineUp.StringerReWork.stringer-rework-request', $data);
	}
	
	
	
	public function StringerRWAll(Request $request)
	{
		$data['menu'] = 'stringer-rework-all';
		$data['empId'] = request()->session()->get('empId');
		...
		*/
        ...
        $data['PermittedMenuList'] = self::PermittedMenuList(request()->session()->get('empId'));
		return view('ProductionLineUp.StringerReWork.stringer-rework-all', $data);
	}

	public function Approval_list(Request $request)
	{
		$data['menu'] = 'stringer-rework-approval-list';
		$data['empId'] = request()->session()->get('empId');
		...
		$data['PermittedMenuList'] = self::PermittedMenuList(request()->session()->get('empId'));
		return view('ProductionLineUp.StringerReWork.stringer-rework-approval-list', $data);
	}

	public function Detailed_list(Request $request)
	{
		$data['menu'] = 'stringer-rework-detailed';
		$data['empId'] = request()->session()->get('empId');
...
ORDER BY psl.created_at DESC;
		...
		$data['PermittedMenuList'] = self::PermittedMenuList(request()->session()->get('empId'));
		return view('ProductionLineUp.StringerReWork.stringer-rework-detailed', $data);
	}

	public function add_stringer_rework(Request $request)
	{
		$data['menu'] = 'stringer-rework';
		...
		$data['DefectCat'] = DB::table('master_type_dtls')
			...
			->get();
		$data['TableMaster'] = DB::table('master_type_dtls')
			->select('master_type_dtls.*')
			->where('master_type_dtls.parent_id', 46)
			->get();
			
		$data['PermittedMenuList'] = self::PermittedMenuList(request()->session()->get('empId'));
		return view('ProductionLineUp.StringerReWork.add-stringer-rework', $data);
	}

	public function view_stringer_rework(Request $request, $id)
	{
		
		$data['menu'] = (isset($_GET['menu']))?$_GET['menu']:'stringer-rework';
		$data['id'] = $id;
		if ($id) {
			$data['stringerReworkDetails'] = DB::table('tbl_factory_stringer_rw_laravel as psl')
				->select(
					'psl.*',
					'sh.shift as shiftdtl',
					'a.fullname AS addByName',
				...
				->where('psl.id', $id)
				->first();
			$data['stringerReworkMaterials'] = Factory_Stringer_ReWork_Material::where('StringerRwId', $id)->get();
          ...
          WHERE phl2.batchNo = phl1.batchNo
				...
				->where('psml.material', 1)
				->where('psml2.material', 2)
				->where('psl.batchNo', $data['stringerReworkDetails']->batchNo)
				->get();
			$data['stringerReworkHistory'] = DB::table('tbl_factory_stringer_rw_history_laravel AS f')
				->leftJoin('mstr_emp AS a', 'f.actionBy', '=', 'a.id')
				->select('f.*', 'a.fullname AS actionBy')
				->where('f.strReworkId', $id)
				->get();
		} else {
			$data['stringerReworkDetails'] = null;
			$data['stringerReworkMaterials'] = [];
		}
		
	    $data['PermittedMenuList'] = self::PermittedMenuList(request()->session()->get('empId'));
		return view('ProductionLineUp.StringerReWork.stringer-rework-view-request', $data);
	}
		...
		if (request()->session()->has('empId')) {
			...
			if ($batchNo) {
				$data['stringerReworkDetails'] = DB::table('tbl_factory_stringer_rw_laravel as psl')
					...
					->first();
				$data['stringerReworkMaterials'] = Factory_Stringer_ReWork_Material::where('StringerRwId', $batchNo)->get();
				$data['batchData'] = DB::table('tbl_factory_production_setup_laravel as psl')
					->select(
						'psl.wattage',
						'psml.size AS efficiency',
          ...
          WHERE phl2.batchNo = phl1.batchNo
			...
			$data['productSetDtls'] = DB::table('tbl_factory_stringer_rw_laravel as psl')
				->select(
					'psl.*',
					'asl.stage_title',
					'b.fullname AS actionByName'
            ...
            WHERE phl2.strReworkId = phl1.strReworkId
				...
				->where('psl.id', $batchNo)
				->get();
			$data['productSetTrail'] = DB::table('tbl_factory_stringer_rw_history_laravel')
				->select('tbl_factory_stringer_rw_history_laravel.*', 'b.fullname')
				->join('mstr_emp AS b', 'tbl_factory_stringer_rw_history_laravel.actionBy', '=', 'b.id')
				->where('tbl_factory_stringer_rw_history_laravel.strReworkId', $batchNo)
				->get();
			$data['menu'] = 'stringer-rework-approval-list';
			
			$data['PermittedMenuList'] = self::PermittedMenuList(request()->session()->get('empId'));
			return view('ProductionLineUp.StringerReWork.approve-details', $data);
		} else {
			return redirect()->to(url(''))->with('authErr', 'Youare not properly logged in.');
		}
	}

	public function store_stringer_rework(Request $request)
	{
		// dd($request->all());
		// $request->validate([
		// 	'date' => 'required',
		...
		$stages = ApprovalStage_Model::where('stage_module', '1753935876')
			->where('stage_stat', '1')
			->orderBy('id', 'asc')
			->limit(1)
			->get();
		...
		$res = Factory_Stringer_ReWork::create($data);
		...
		if ($res->exists) {
			...
			foreach ($materials as $key => $value) {
				...
				Factory_Stringer_ReWork_Material::create($materialData);
			}
			...
			Factory_Stringer_ReWork_History::create($logData);

			return redirect('production-lineup/stringer-rework')->with('success', 'Stringer ReWork Request Created Successfully');
		} else {
			return redirect('production-lineup/stringer-rework')->with('failed', 'Something went wrong, please contact the System Administrator');
		}
	}
		...
		if (request()->session()->has('empId')) {
			...
			if (request()->input('ApprStat') == 1) {
				...
				if (count($nextPositionIdObj) > 0) {
					...
					$result = Factory_Stringer_ReWork::where('id', $batchNo);
			...
			if ($res == 1) {
				...
				$res = Factory_Stringer_ReWork_History::create($data);

				return redirect()->to(url('production-lineup/stringer-rework-approval-list'))->with('success', 'Approval Action done Successfully');
			} else {
				return redirect()->to(url('production-lineup/stringer-rework-approval-list'))->with('failed', 'Something went wrong contact with System Aministrator');
			}
		} else {
	...
	public function formUpdateView($id)
	{
		if (request()->session()->has('empId')) {
			...
			$data['stringerRwMaterials'] = DB::table('tbl_factory_stringer_rw_material_laravel as psml')
				->select(
					'psml.*',
			...
			$data['stringerRwDetails'] = Factory_Stringer_ReWork::find($id);
          ...
          WHERE phl2.batchNo = phl1.batchNo
			...
			$data['batchData'] = DB::table('tbl_factory_production_setup_laravel as psl')
				...
				->join('tbl_factory_stringer_rw_laravel as sol', 'psl.batchNo', '=', 'sol.batchNo')
				->orderBy('psl.created_at', 'DESC')
				->where('sol.id', $id)
				->first();
				
			$data['PermittedMenuList'] = self::PermittedMenuList(request()->session()->get('empId'));
			return view('ProductionLineUp.StringerReWork.stringer-rework-form-update', $data);
		} else {
			return redirect()->to(url(''))->with('authErr', 'You are not properly logged in.');
		}
	}

	public function updateStringerRework(Request $request, $id)
	{
		$stage = ApprovalStage_Model::where('stage_module', '1753935876')
			->where('stage_stat', '1')
			->orderBy('id', 'asc')
			->first();
		...
		if (Factory_Stringer_ReWork::where('id', $id)->update($data)) {
			if ($request->has('materials')) {
				$materialIds = [];
				foreach ($request->materials as $material) {
					if (isset($material['id']) && !empty($material['id'])) {
						Factory_Stringer_ReWork_Material::where('id', $material['id'])->update([
							'batchNo'         => $request->batch_no,
							'material'        => $material['name'],
							'tableNo'     => $material['table_no'],
							'time'            => $material['time'],
						...
						$newMaterial = Factory_Stringer_ReWork_Material::create([
							'batchNo'         => $request->batch_no,
							'StringerRwId'    => $id,
							'material'        => $material['name'],
							'tableNo'     => $material['table_no'],
				...
				Factory_Stringer_ReWork_Material::where('StringerRwId', $id)
			...
			return redirect()->to(url('production-lineup/stringer-rework'))->with('success', 'Stringer Rework Request Updated Successfully');
		} else {
			return redirect()->to(url('production-lineup/stringer-rework'))->with('failed', 'Something went wrong, please contact the System Administrator');
		}
	}
}
