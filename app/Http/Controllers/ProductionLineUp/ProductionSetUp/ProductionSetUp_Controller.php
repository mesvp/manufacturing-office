...
namespace App\Http\Controllers\ProductionLineUp\ProductionSetUp;
...
use App\Models\ProductionLineUp\{materialMaster_Model, ProductSetUp_Model, ProductSetUpMaterial_Model, ProductionSetUpHist_Model};
use App\Models\ApprovalMatrix\ApprovalStage_Model;
use App\Models\ApprovalMatrix\Approver_Model;
use Session;
class ProductionSetUp_Controller extends Controller
{
  ...
  public  static function PermittedMenuList($sessionId){
    ...
    public function ProductionAll(Request $request)
	{
		$data['menu'] = 'production-setup-all';
      $data['PermittedMenuList'] = self::PermittedMenuList(request()->session()->get('empId'));
		$data['empId'] = request()->session()->get('empId');
		...
		return view('ProductionLineUp.ProductionSetUp.production-setup-all', $data);
	}
    ...
    {
      // It's better to use $request->input() or $request->q for Laravel standards
      $finishedGoodHidden = $request->get('q');
      Session::put('sessBushing_finishGood', $finishedGoodHidden);
    
      if (!$finishedGoodHidden) {
          return response('<option value="">No Materials Found</option>');
      }
      ...
      $MAT_DATA = DB::table('bom')
          ...
          ->groupBy(
              ...
              'prj_material.material_name' // Added for SQL strictly grouped modes
          )
          ->get();
    
      $output = '<option value="">Select BOM Material</option>';
      
      foreach($MAT_DATA as $data){
          $output .= '<option value="'.$data->Raw_Material.'">'.$data->Raw_Material_Name.'</option>';
      }
      
      return response($output);
    }
    
    public function getFinishedGoodDataAjax2(Request $request)
    {
      // It's better to use $request->input() or $request->q for Laravel standards
      $finishedGoodHidden = $request->get('q');
      //Session::put('sessBushing_finishGood', $finishedGoodHidden);
      ...
      $MAT_DATA = DB::table('bom')
          ...
          ->where('materialmanagement_add_material.Approve_status', 'APPROVE')
          ->where('bom.Approve_status', 'APPROVE')
          ->where('bom.Raw_Material_FG', '=', Session::get('sessBushing_finishGood'))
          ->where('bom_material.Material', '=', $finishedGoodHidden)
    
          ->orderBy('bom_material.updated_at', 'DESC')
          ->limit(1)
          ->groupBy(
              'bom_material.id',
              'bom_material.Material',
	...
	public function approveDtls(Request $request)
	{
      ...
      $data['PermittedMenuList'] = self::PermittedMenuList(request()->session()->get('empId'));
			return view('ProductionLineUp.ProductionSetUp.approve-details', $data);
		} else {
			return redirect()->to(url(''))->with('authErr', 'You are not properly logged in.');
		}
	}

	public function getMatUOMAjax($id)
	{
		$material = materialMaster_Model::find($id);
		return response()->json(['uom' => $material ? $material->uom : '']);
	}
	...
	public function formUpdateView($batchNo)
	{
		if (request()->session()->has('empId')) {
			$data['menu'] = 'production-setup';
			$data['empId'] = request()->session()->get('empId');
			$data['productSetDtls'] = ProductSetUp_Model::where('batchNo', $batchNo)->first();
			$data['productSetMtrl'] = ProductSetUpMaterial_Model::where('batchNo', $batchNo)->get();
			$data['ShiftMaster'] = DB::table('hr_mstr_shift')
				->select('hr_mstr_shift.*')
				->get();
			$data['PlantMaster'] = DB::table('master_type_dtls')
      ...
      $data['PermittedMenuList'] = self::PermittedMenuList(request()->session()->get('empId'));
			return view('ProductionLineUp.ProductionSetUp.production-setup-form-update', $data);
		} else {
			return redirect()->to(url(''))->with('authErr', 'You are not properly logged in.');
		}
	}

	public function updateProduction($batchNo)
	{
		if (request()->session()->has('empId')) {
			$stage = ApprovalStage_Model::where('stage_module', '1752154155')
			->where('stage_stat', '1')
			->orderBy('id', 'asc')
			->first();
			$batchNo = request()->input('batchNo');
			...
			$result = ProductSetUp_Model::where('batchNo', $batchNo);
			$res = $result->update($data);

			if ($res == 1) {
				ProductSetUpMaterial_Model::where('batchNo', $batchNo)->delete();
				...
				return redirect()->to(url('production-lineup/production-setup'))->with('success', 'Schedule updated Successfully');
			} else {
				return redirect()->to(url('production-lineup/production-setup'))->with('failed', 'Something went wrong contact with System Aministrator');
			}
		} else {
