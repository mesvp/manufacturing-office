    ...
    public function Plant_Machinery(Request $request)
    {
        ...
        if (isset($res['Plant_Name']) && $res['Plant_Name'] != '') {
            foreach ($res['Plant_Name'] as $key => $val) {
                ...
                if ($edit != '') {
                    $prj_apprvsts=Factory_Plant_Machinery::select('factory_address_details.Approve_status')
                        ->leftJoin('factory_address_details','factory_plant_machineries.factory_id','=','factory_address_details.id')
                        ->where('factory_plant_machineries.Plant_Name',$res['Plant_Name'][$key])
                        ->orderBy('factory_plant_machineries.id', 'DESC')->first();
                    if ($prj_apprvsts !== null ) {
                        if($prj_apprvsts->Approve_status=="" || $prj_apprvsts->Approve_status=="APPROVE" || $prj_apprvsts->Approve_status=="RECHECK"){
                            //return redirect('FactoryCreater/List')->withErrors( 'Already Added...');
                            return redirect('FactoryCreater/step5')->withErrors( 'Already Added Plant...');
                        }
                        else {
                            $plantmachinary = Factory_Plant_Machinery::where('id', $edit)->update(['Plant_Name' => $res['Plant_Name'][$key] ?? '', 'Production_Capacity' => $res['Production_Capacity'][$key] ?? '', 'Material_Name' => $res['Material_Name'][$key] ?? '','uom_prd' => $res['uom_prd'][$key] ?? '', 'Date_Of_Purchase' => $res['Date_Of_Purchase'][$key] ?? '', 'Machine_Company_Name' => $res['Machine_Company_Name'][$key] ?? '', 'Remarks' => $res['Remarks'][$key] ?? '']);
                        }
                    }else{
                        $plantmachinary = Factory_Plant_Machinery::where('id', $edit)->update(['Plant_Name' => $res['Plant_Name'][$key] ?? '', 'Production_Capacity' => $res['Production_Capacity'][$key] ?? '', 'Material_Name' => $res['Material_Name'][$key] ?? '','uom_prd' => $res['uom_prd'][$key] ?? '', 'Date_Of_Purchase' => $res['Date_Of_Purchase'][$key] ?? '', 'Machine_Company_Name' => $res['Machine_Company_Name'][$key] ?? '', 'Remarks' => $res['Remarks'][$key] ?? '']);
                    }
                    ...
                    // if (isset($res['Material_Name'][$key]) && $res['Material_Name'][$key] != '') {
                    //     $plantmachinary->Material_Name = $res['Material_Name'][$key];
                    // }
                    if (isset($res['uom_prd'][$key]) && $res['uom_prd'][$key] != '') {
                        $plantmachinary->uom_prd = $res['uom_prd'][$key];
                    }
                    ...
                    // $prj_apprvsts=Factory_Address_Detail::select('factory_address_details.Approve_status')
                    //     ->where('name_of_unit',$request->name_of_unit)
                ...
                if (isset($res['Machine_Name'][$key]) && $res['Machine_Name'][$key] != '') {
                    foreach ($res['Machine_Name'][$key] as $key1 => $val1) {
                        ...
                        if ($machinenameID != '') {
                            ...
                            if (isset($res['Machine_description'][$key][$key1]) && $res['Machine_description'][$key][$key1] != '') {
                                $machinenames->Machine_description = $res['Machine_description'][$key][$key1];
                            }
                ...
                if (isset($res['Warranty'][$key]) && $res['Warranty'][$key] != '') {
                    foreach ($res['Warranty'][$key] as $key2 => $val2) {
                        ...
                        if ($warrntyID != '') {
                            ...
                            if ($request->file('Attachement_warrenty' . $key . $key2) != '') {
                                $name = $request->file('Attachement_warrenty' . $key . $key2)->getClientOriginalName();
                                $warranty->Attachement_warrenty = $request->file('Attachement_warrenty' . $key . $key2)->storeAs('PlantMachin_warrenty', $name);
                            }
                ...
                if (isset($res['Add_Field_Name_Manually'][$key]) && $res['Add_Field_Name_Manually'][$key] != '' || isset($res['otherID'][$key]) && $res['otherID'][$key] != '') {
                    foreach ($res['Add_Field_Name_Manually'][$key] as $key3 => $val3) {
                        ...
                        if ($otherID != '') {
                            ...
                            if (isset($res['Add_Field_Name_Manually'][$key][$key3]) && $res['Add_Field_Name_Manually'][$key][$key3] != '') {
                                $others->Add_Field_Name_Manually = $res['Add_Field_Name_Manually'][$key][$key3];
                            }
                            if (isset($res['Enter_Manually_Details'][$key][$key3]) && $res['Enter_Manually_Details'][$key][$key3] != '') {
