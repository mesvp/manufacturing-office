...
class EmployeeGatePassApprovalController extends Controller
{
    public function EmployeeGatepassApproveList(Request $request)
    {
        ...
        if (isset($CUSTEXT[2]['Forward']) || isset($CUSTEXT[2]['approver'])) {
            ...
            return view('GatePass.Employee_Gatepass_Approval', ['employeedata' => [], 'DropdownData' => [], 'empName' => [], 'fromdate' => $fromdate, 'todate' => $dateto, 'RequestNos' => '', 'RequestBys' => '', 'GatePassNos' => '', 'RequestOutTimes' => '', 'RequestInTimes' => '', 'Manufacturing_unitdata' => [], 'plant_namedata' => [], 'CostCenter' => '', 'sub_CostCenter' => '', 'SecurityGuards' => '', 'PersonWithVehicles' => '', 'Projects' => [], 'Sub_Projects' => []]);
        }
    ...
    public function approve(Request $request)
    {
        ...
        if (!empty($request->during_approval)) {
            Gatepass_Employee::where('request_no', $request->req_no)->update(['Approve_status' => $request->during_approval]);
        ...
        if ($request->during_approval === 'APPROVE') {
            ...
            Gatepass_Employee::where('request_no', $request->req_no)->update(['Forward_Status' => 0]);
            ...
            if ($check->Approve_Step == 1 && $DepartStepcount2 > 0) {
                Gatepass_Employee::where('request_no', $request->req_no)->update(['Approve_Step' => 2, 'Approve_status' => null]);
                $prod=false;
            }

            if ($check->Approve_Step == 2 &&  $DepartStepcount3 > 0) {
                Gatepass_Employee::where('request_no', $request->req_no)->update(['Approve_Step' => 3, 'Approve_status' => null]);
                $prod=false;
            }
        }
        ...
        if ($request->during_approval === 'FORWARD') {
            ...
            Gatepass_Employee::where('id', $request->approveID)->update(['Forward_Status' => 1]);
        ...
        if ($request->during_approval == '' && $request->pre_post_approval == '') {
            Gatepass_Employee::where('id', $request->approveID)->update(['Approve_status' => null]);
            ...
            return redirect('GatePass/List')->with('success', 'successfull.....');
        } else {
            return redirect('GatePass/Employee_Gatepass_Approval')->with('success', 'Approved successfully.....');
        }
    }
    public function Release_Hold(Request $request, $id)
    {
        ...
        $emp_gatepass =  Gatepass_Employee::where('request_no', $id)->update(['Approve_status' => null]);
    ...
    public function ExportEmployeeApproval(Request $request)
    {
