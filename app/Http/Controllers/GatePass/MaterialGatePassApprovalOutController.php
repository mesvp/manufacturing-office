...
use App\Models\GatePass\{OutGatepassMaterials, MaterialGatePassApproval, Forwarded_Data_Gatepass};
use App\Models\Master\{Master_Type, Master_Type_Details, Master_Request_Type};
use Illuminate\Support\Facades\Session;

class MaterialGatePassApprovalOutController extends Controller
{
    public function MaterialGatepassOut_ApproveList(Request $request)
    {
        ...
        if (isset($CUSTEXT[2]['Forward']) || isset($CUSTEXT[2]['approver'])) {
            $query = OutGatepassMaterials::query();
        } else {
            return view('GatePass.Material_Gatepass_Out_Approval',['materialdata' => [], 'DropdownData' => [], 'fromdate' => '', 'todate' => '', 'RequestNos' => [], 'RequestBys' => [], 'VehicleNos' => [], 'insuranceNos' => [], 'invoicechallannos' => [], 'drivernames' => [], 'DriverNumbers' => [], 'BillNos' => []]);
        }

        if (isset($CUSTEXT[2]['inputer']) || auth()->user()->role == 0) {
            $query = OutGatepassMaterials::orderBy('id', 'DESC');
        } else {
            $query = OutGatepassMaterials::where('status', 0)->orderBy('id', 'DESC');
        }
        ...
        $ForDropdown = OutGatepassMaterials::orderBy('id', 'DESC')->get();
        ...
        if (isset($CUSTEXT[2]['Forward']) && isset($CUSTEXT[2]['approver'])){
            ...
            ->orWhere(function ($query) {
                $query->whereRaw('request_no IN (SELECT DataID FROM forwarded_data_gatepass WHERE Forward_To_id="' . auth()->user()->id . '" AND status=0) AND (Out_Approve_status IS NULL OR Out_Approve_status="FORWARD") AND `Out_Forward_Status` = 1');
            })
            ->orWhereRaw('request_no IN (SELECT DataID FROM forwarded_data_gatepass WHERE Forward_To_id="' . auth()->user()->id . '" AND status=0) AND (Out_Approve_status IS NULL OR Out_Approve_status="FORWARD") AND `Out_Forward_Status` = 1')
            ->orderBy('id', 'DESC');
        }
        elseif (isset($CUSTEXT[2]['Forward'])){
            $query = $query->where('Out_Forward_Status', 1)->whereRaw('request_no IN (SELECT DataID FROM forwarded_data_gatepass WHERE Forward_To_id="' . auth()->user()->id . '" AND status=0) AND (Out_Approve_status IS NULL OR Out_Approve_status="FORWARD")')->orderBy('id', 'DESC');
        }
    ...
    public function Out_approve(Request $request)
    {
        ...
        if (!empty($request->during_approval)) {
            OutGatepassMaterials::where('request_no', $request->req_no)->update(['Out_Approve_status' => $request->during_approval]);
            MaterialGatePassApproval::where('GatepassID', $request->req_no)->where('status', 1)->update(['status' => 0]);
        }
        $check = OutGatepassMaterials::find($request->approveID);
        if ($request->during_approval === 'APPROVE') {
            $status = Forwarded_Data_Gatepass::where('DataID', $request->req_no)->update(['status' => 1]);
            OutGatepassMaterials::where('request_no', $request->req_no)->update(['Out_Forward_Status' => 0]);
            ...
            if ($check->Out_Approve_Step == 1 && $DepartStepcount2 > 0) {
                OutGatepassMaterials::where('request_no', $request->req_no)->update(['Out_Approve_Step' => 2, 'Out_Approve_status' => null]);
                $prod=false;
            }

            if ($check->Out_Approve_Step == 2 &&  $DepartStepcount3 > 0) {
                OutGatepassMaterials::where('request_no', $request->req_no)->update(['Out_Approve_Step' => 3, 'Out_Approve_status' => null]);
                $prod=false;
            }
        }
        if ($request->during_approval === 'REJECT') {
            $prod=OutGatepassMaterials::find($request->approveID);
        }
        ...
        if ($request->during_approval === 'FORWARD') {
            ...
            OutGatepassMaterials::where('id', $request->approveID)->update(['Out_Forward_Status' => 1]);
        ...
        if ($request->during_approval == '' && $request->pre_post_approval == '') {
            OutGatepassMaterials::where('id', $request->approveID)->update(['Out_Approve_status' => null]);
            ...
            return redirect('GatePass/Material_Gatepass_Out_Approval')->with('success', 'Approved successfully.....');
        }
    }
    ...
    public function Out_Release_Hold(Request $request, $id)
    {
        ...
        $mtrl_gatepass =  OutGatepassMaterials::where('request_no', $id)->update(['Out_Approve_status' => null]);
