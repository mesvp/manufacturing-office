...
class VisitorGatePassApprovalOutController extends Controller
{
    public function VisitorGatepassOut_ApproveList(Request $request)
    {
    ...
    public function Out_approve(Request $request)
    {
        ...
        if (!empty($request->during_approval)) {
            Gatepass_Visitor::where('out_request_no', $request->req_no)->update(['Out_Approve_status' => $request->during_approval]);
        ...
        if ($request->during_approval === 'APPROVE') {
            ...
            Gatepass_Visitor::where('out_request_no', $request->req_no)->update(['Out_Forward_Status' => 0]);
            ...
            if ($check->Out_Approve_Step == 1 && $DepartStepcount2 > 0) {
                Gatepass_Visitor::where('out_request_no', $request->req_no)->update(['Out_Approve_Step' => 2, 'Out_Approve_status' => null]);
                $prod=false;
            }

            if ($check->Out_Approve_Step == 2 &&  $DepartStepcount3 > 0) {
                Gatepass_Visitor::where('out_request_no', $request->req_no)->update(['Out_Approve_Step' => 3, 'Out_Approve_status' => null]);
                $prod=false;
            }
        }
        ...
        if ($request->during_approval === 'FORWARD') {
            ...
            Gatepass_Visitor::where('id', $request->approveID)->update(['Out_Forward_Status' => 1]);
        ...
        if ($request->during_approval == '' && $request->pre_post_approval == '') {
            Gatepass_Visitor::where('id', $request->approveID)->update(['Out_Approve_status' => null]);
            ...
            return redirect('GatePass/Visitor_Gatepass_Out_Approval')->with('success', 'Approved successfully.....');
        }
    }
    ...
    public function Out_Release_Hold(Request $request, $id)
    {
        ...
        $visitor_gatepass =  Gatepass_Visitor::where('out_request_no', $id)->update(['Out_Approve_status' => null]);
