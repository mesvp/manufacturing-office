...
class RawMaterialViewController extends Controller
{
    ...
    public function AddRawMaterial($id = null)
    {
        ...
        return view('RawMaterial/RawMaterial', ['edit' => $edit, 'raw' => $raw_arr, 'Manufacturing_Unit' => $Manufacturing_Unit, 'Organization' => $Organization, 'raw_count' => $raw_count, 'raw_data_count' => $raw_data_count, 'Godown_Name' => $Godown_Name, 'Raw_Material' => $Raw_Material, 'OB' => $OB, 'Rack_No' => $Rack_No, 'Sub_Rack_No' => $Sub_Rack_No, 'Bin_No' => $Bin_No, 'Sub_Bin_No' => $Sub_Bin_No, 'UOM' => $UOM]);
    }
    ...
    public function DownloadFilteredData(Request $request)
    {
        ...
        foreach ($Alldata_arr as $key => $val) {
            $rowData = [
                ...
                "Organization" => isset($val->Organization->organization) && $val->Organization->organization != '' ? $val->Organization->organization : '',
                ...
                "Godown Name" => isset($val->Godown_Name->Godown_Name) && $val->Godown_Name->Godown_Name != '' ? $val->Godown_Name->Godown_Name : '',
                "Date" => isset($val->raw->date) && $val->raw->date != '' ? $val->raw->date : '',
                "Raw Material" => isset($val->Raw_Material->Material_Name) && $val->Raw_Material->Material_Name != '' ? $val->Raw_Material->Material_Name : '',
                "HSN Code" => isset($val->rawss->HSN_Code) && $val->rawss->HSN_Code != '' ? $val->rawss->HSN_Code : '',
                "UOM" => isset($val->UOM->UOMs) && $val->UOM->UOMs != '' ? $val->UOM->UOMs : '',
                "OB" => isset($val->OB->OB) && $val->OB->OB != '' ? $val->OB->OB : '',
                "Received QTY" => isset($val->rawss->Received_QTY) && $val->rawss->Received_QTY != '' ? $val->rawss->Received_QTY : '',
                "Balance Stock" => isset($val->rawss->Balance_Stock) && $val->rawss->Balance_Stock != '' ? $val->rawss->Balance_Stock : '',
    ...
    {
        $data = MaterialManagement_Add_Material::find($id);
